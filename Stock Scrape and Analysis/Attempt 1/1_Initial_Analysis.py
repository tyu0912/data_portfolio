...
for stock in stocklist:
    ...
    if PE < 25 and price < 35 and ROE > 15 and ROA > 7.5:
        #print stock
...
for stock in finalreport[:]:
    if lala == 1: # stock[MOA50MOA20DIF] > statistics.mean(UpTrended) and ((0-statistics.stdev(UpTrendedNorm))/2 < stock[MOA50MOA20NORMDIF] < (0+statistics.stdev(UpTrendedNorm))/2):

        for item in stock:
            if stock.index(item) == (len(stock) - 1):
                output = str(item) + '\n'
            else:
                output = str(item) + ', '
